...
const TOOLS: { key: ToolKey; label: string; description: string }[] = [
  ...
  { key: "performance", label: "Site Performance", description: "Real response time, page weight, and crawlability signals â€” no fabricated traffic numbers." },
    ...
    <ul className="mt-4 space-y-2">
      {checks.map((c) => (
        <li key={c.label} className="flex gap-3 text-sm">
          <span className={c.passed ? "text-teal" : "text-amber"}>{c.passed ? "âœ“" : "!"}</span>
          <span>
            <span className="text-fg">{c.label}</span>{" "}
            <span className="text-muted">â€” {c.detail}</span>
          </span>
        </li>
      ))}
    </ul>
      ...
      <div className="container-page">
        ...
        >
          ...
          <p className="mt-4 text-muted leading-relaxed">
            These run against the real page â€” the backend fetches it and
          ...
          <div className="border border-line rounded-lg p-6 sm:p-8">
            <form onSubmit={run} className="flex flex-col sm:flex-row gap-3">
              ...
              <button
                type="submit"
                disabled={loading}
                className="px-6 py-3 rounded bg-[#D36938] text-ink font-medium hover:bg-[#D36938]/90 transition-colors disabled:opacity-50 shrink-0"
              >
                ...
                >
                  ...
                  {"speed_rating" in result && (
                    <div className="grid grid-cols-3 gap-4 mt-6 text-sm">
                      <div><div className="text-muted text-xs mb-1">Speed</div><div className="font-medium">{result.speed_rating} Â· {result.page.response_time_ms}ms</div></div>
                      <div><div className="text-muted text-xs mb-1">Weight</div><div className="font-medium">{result.weight_rating} Â· {result.page.page_size_kb}KB</div></div>
